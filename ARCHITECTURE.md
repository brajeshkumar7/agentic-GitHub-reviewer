...
| `ExecutionEngine` | Sole stateful executor. Validates its run preconditions and tool calls, selects ready steps in stable plan order, checks dependencies and budgets, dispatches through the registry, stores typed results, and emits events. Current baseline permits one transient retry, marks permanent failures, skips blocked dependents, and continues independent steps; advanced fallback/replanning remains deferred. | `execute(plan: ValidatedPlan, state: AgentState) -> AgentState` |
...
- The current executor retries one classified transient tool failure at most once, then marks the step failed. It skips steps whose dependencies did not succeed and continues independent steps. It does not yet select fallback calls or request a replan; those actions remain reserved for the later recovery phase.
