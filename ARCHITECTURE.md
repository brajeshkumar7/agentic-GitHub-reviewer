...
- The executor delegates failures to `FailureHandler`; eligible failures can retry, use a prevalidated fallback, request one validated revision, or mark the step failed. Failed dependencies are skipped; independent steps continue.
- Successful tool outputs and typed failures are retained in `AgentState.tool_results`; conversion of fetched pages into verified `Evidence` belongs to the evidence phase.
- Fixed limits: 8 plan steps, 20 tool invocations per run, 3 calls per logical step (initial plus at most two retries; fallback and revised replacements consume the same budget), 2 attempts per model operation, and at most one validated-plan revision. A malformed structured response may use the model operation's second attempt before a Plan exists; revision does not reset budgets. A replacement inherits its predecessor's attempt count.
- `AGENT_INJECT_FAILURE` enables at most one configured demo failure (`tool_timeout` or `malformed_tool_response`) for an optional allowlisted tool. It returns a normal typed failure, emits an injection event, and enters ordinary recovery.
...
1. **Retry:** For transient timeout, connection error, HTTP 5xx, temporary 429, or explicitly retryable malformed provider output, repeat the same validated call up to twice if the step and run budgets permit. Do not alter arguments during a retry.
2. **Fallback:** For an eligible semantic failure, use the step's one predeclared, PlanValidator-approved fallback. A fallback consumes one call from the same three-call logical-step budget.
3. **Replan:** If a semantic/source failure remains recoverable, the failed logical step has a call slot left, and the single validated-plan revision allowance remains, ask Planner for one replacement step. Validate the revised plan; preserve completed work and all attempt/call budgets. The replacement carries `recovery_of_step_id` and inherits the failed step's attempt count. Replanning does not authorize unsafe tools or repeat completed steps.
4. **Permanently fail:** Mark the step `FAILED` when failure is non-retryable and no approved fallback/replan applies or a budget is exhausted. Mark dependent steps `SKIPPED`; continue independent work if useful, then return `PARTIAL` or `FAILED`. An invalid plan after the single replan is a run-level `FAILED` terminal state.
...
`{call_id: UUID, run_id: UUID, step_id: str, tool_name: ToolName, arguments: validated tool-specific input model, attempt: 1..3, kind: primary | retry | fallback}`. `tool_name` selects only one of the three fixed schemas; mismatched arguments fail validation. Engine supplies identity/attempt/kind; planner only proposes step input and tool name.
...
`{evidence_id: UUID, goal_id: UUID, step_id: str, source_url: canonical public HTTPS URL, title: str, publisher: str | null, source_type: primary | independent_secondary | other, source_group_id: str, published_at: UTC datetime | null, retrieved_at: UTC datetime, supporting_text: 1..20,000 chars, supports: list[str], verification: candidate | verified | contradictory}`. Search snippets alone cannot become `verified` evidence. Provenance identifies the goal and plan step that produced the fetched evidence.
...
`FinalReport` uses the requested fields `{goal, status, plan, execution_summary, failures, recoveries, evidence, findings, limitations, sources}`. `ReportPlanStep` carries ID/objective/tool/status/dependencies; `ReportExecutionSummary` carries total/completed/failed/retries; `Finding` carries a title, summary, and verified evidence IDs. Sources must be an exact projection of stored evidence, and every finding reference must resolve to verified evidence. Only deterministic application code supplies `status` and `sources`. A completed report requires at least one supported finding. `to_json()` serializes the validated model; `to_markdown()` renders the required brief sections and distinguishes observed evidence from generated synthesis and limitations.
